...
class ImageService {
  ...
  getImageUrl(imagePath, options = {}) {
    ...
    const fileName = imagePath
      .replace(/^.*[\\/]/, '') // 将 [\\\/] 改为 [\\/]
      .replace(/^images[\\/]/, ''); // 同样将 [\\\/] 改为 [\\/]

    // 自动分类
    const finalCategory =
      category === 'auto' ? this.categorizeImage(fileName) : category;
  ...
  getCriticalIcons() {
    const icons = {
      ...
      // 分类封面
      aiCover: this.getImageUrl('CS.AI Logo - Artificial Intelligence.png', {
        category: 'large',
      }),
      cvCover: this.getImageUrl('CS.CV Logo - Computer Vision.png', {
        category: 'large',
      }),
      lgCover: this.getImageUrl('CS.LG Logo - Learning.png', {
        category: 'large',
      }),
    };
