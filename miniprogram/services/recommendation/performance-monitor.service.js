...
class RecommendationPerformanceMonitor {
  ...
  calculateQualityMetrics(testSet, recommendations) {
    ...
    this.metrics.recall =
      totalRelevant > 0 ? relevantRecommended / totalRelevant : 0;

    // 计算覆盖率 (Coverage)
    const uniqueRecommended = new Set(recommendations.map(r => r.podcastId))
      .size;
  ...
  updateClickThroughRate() {
    const recentRecommendations = this.interactions.filter(
      i =>
        i.type === 'recommendation' &&
        i.timestamp > Date.now() - 24 * 60 * 60 * 1000 // 最近24小时
    );

    const recentClicks = this.interactions.filter(
      i => i.type === 'click' && i.timestamp > Date.now() - 24 * 60 * 60 * 1000
    );
  ...
  updateConversionRate() {
    ...
    const recentConversions = this.interactions.filter(
      i =>
        i.type === 'conversion' &&
   ...
   */
  updateCacheHitRate() {
    const totalHits = Object.values(this.cacheHits || {}).reduce(
      (sum, count) => sum + count,
      0
    );
    const totalMisses = Object.values(this.cacheMisses || {}).reduce(
      (sum, count) => sum + count,
      0
    );
  ...
  getPerformanceStats() {
    const avgResponseTime =
      this.metrics.responseTime.length > 0
        ? this.metrics.responseTime.reduce((sum, r) => sum + r.duration, 0) /
          this.metrics.responseTime.length
        : 0;

    const recentResponseTimes = this.metrics.responseTime.filter(
      r => r.timestamp > Date.now() - 60 * 60 * 1000 // 最近1小时
    );

    const recentAvgResponseTime =
      recentResponseTimes.length > 0
        ? recentResponseTimes.reduce((sum, r) => sum + r.duration, 0) /
  ...
  calculateOverallHealth(stats) {
    ...
    const performanceScore = Math.max(
      0,
      1 - stats.performance.recentAvgResponseTime / 3000
    );
    const qualityScore =
      (stats.quality.precision +
        stats.quality.recall +
        stats.quality.diversity) /
      3;
