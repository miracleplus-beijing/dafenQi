...
class InsightService {
  ...
  formatInsightData(rawInsight) {
    try {
      return {
        ...
        related_papers: Array.isArray(rawInsight.related_papers) ? rawInsight.related_papers : [],
        related_authors: Array.isArray(rawInsight.related_authors) ? rawInsight.related_authors : [],
  ...
  getDefaultInsightData(podcastId = null) {
    const defaultData = {
      id: 'default',
      title: '认知提取',
      summary: '本期播客内容丰富，涵盖了当前热门话题的深度分析，为听众提供了独特的观点和洞察。',
      detailed_content: '节目深入探讨了相关主题，通过专业的分析和生动的案例，帮助听众更好地理解复杂的概念和现象。内容具有很强的实用性和启发性。',
  ...
  async incrementViewCount(insightId) {
    try {
      // 先获取当前浏览次数
      const currentResult = await this.supabaseRequest(`insights?id=eq.${insightId}&select=view_count`)
  ...
  async incrementLikeCount(insightId) {
    try {
      // 先获取当前点赞次数
      const currentResult = await this.supabaseRequest(`insights?id=eq.${insightId}&select=like_count`)
  ...
  async getUserInteractions(userId, insightId) {
    try {
      ...
      const result = await this.supabaseRequest(`user_insight_interactions?${query}`)
