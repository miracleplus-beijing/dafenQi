...
class AudioPreloadTester {
  ...
  async testChunkManager() {
    ...
    const testAudioUrl =
      'https://gxvfcafgnhzjiauukssj.supabase.co/storage/v1/object/public/podcast-audios/2025.8/8.1.mp3';

    try {
      // 测试音频文件分析
      const audioInfo = await audioChunkManager.analyzeAudioFile(
        testAudioUrl,
        1800
      ); // 30分钟音频
      ...
      const tests = {
        ...
        chunkCalculation:
          audioChunkManager.getChunkIndexByTime(testAudioUrl, 900) >= 0, // 15分钟位置
        byteRange: (() => {
          const range = audioChunkManager.getChunkByteRange(testAudioUrl, 0);
          return range.start === 0 && range.end > 0;
        })(),
        preloadChunks:
          audioChunkManager.getPreloadChunks(testAudioUrl, 900, 3).length > 0,
      };
  ...
  async testSmartPreloadController() {
    ...
    try {
      ...
      const tests = {
        ...
        statsAvailable:
          stats.loadingStats &&
          typeof stats.loadingStats.totalLoads === 'number',
      };
    ...
    try {
      const testPodcasts = [
        ...
        },
        {
          ...
          title: '测试播客2',
          audio_url:
            'https://gxvfcafgnhzjiauukssj.supabase.co/storage/v1/object/public/podcast-audios/2025.8/8.2.mp3',
          duration: 2100,
        },
      ];
      ...
      const tests = {
        ...
        statsAvailable:
          stats.chunk && typeof stats.totalMemoryUsage === 'string',
        bufferProgress: (() => {
          const progress = audioPreloaderService.getBufferProgress(
            testPodcasts[0].audio_url,
            900,
            1800
          );
  ...
  async testPerformanceMetrics() {
    ...
    try {
      ...
      const tests = {
        ...
        cacheHitRate:
          cacheStats.hitRate === '0.00%' || parseFloat(cacheStats.hitRate) >= 0, // 命中率有效
        networkAdaptive:
          controllerStats.adaptiveConfig &&
          controllerStats.adaptiveConfig.networkSpeed !== 'unknown',
        loadingPerformance:
          controllerStats.loadingStats.averageLoadTime < 5000 ||
          controllerStats.loadingStats.totalLoads === 0,
        chunkSizeOptimal:
          chunkStats.currentChunkSizeKB >= 200 &&
          chunkStats.currentChunkSizeKB <= 400,
      };
