...
class AudioPreloaderService {
  ...
  async initialize(podcastList, currentIndex = 0) {
    ...
    if (podcastList[currentIndex]) {
      ...
      if (currentPodcast && currentPodcast.audio_url) {
        ...
        try {
          await smartPreloadController.initialize(
            currentPodcast.audio_url,
            currentPodcast.duration || 0
          );
        } catch (error) {
  ...
  async triggerNextAudioPreload(currentIndex) {
    ...
    try {
      ...
      // 分析下一个音频文件并开始预加载初始块
      await audioChunkManager.analyzeAudioFile(
        nextPodcast.audio_url,
        nextPodcast.duration || 0
      );
      ...
      for (const chunkIndex of initialChunks) {
        smartPreloadController.enqueuePreload(
          nextPodcast.audio_url,
          chunkIndex,
          'low'
        ); // 低优先级
    ...
    if (this.podcastList && this.podcastList[newIndex]) {
      ...
      if (newPodcast && newPodcast.audio_url) {
        ...
        if (
          ...
          this.currentPlayingAudio.audio_url !== newPodcast.audio_url
        ) {
          this.currentPlayingAudio = newPodcast;
          console.log('切换到新音频，启用分块预加载:', newPodcast.title);

          try {
            await smartPreloadController.switchAudio(
              newPodcast.audio_url,
  ...
  cleanDistantAudioCache(currentIndex) {
    ...
    audioUrls.forEach(audioUrl => {
      // 找到这个音频在播客列表中的位置
      const audioIndex = this.podcastList.findIndex(
        podcast => podcast.audio_url === audioUrl
      );
      ...
      if (audioIndex >= 0) {
        ...
        if (distance > maxDistance) {
          ...
          if (removedCount > 0) {
            console.log(
              `🧹 清理过远音频缓存: 索引${audioIndex}, 距离${distance}, 清理${removedCount}个块`
            );
          }
        }
      }
  ...
  calculateChunkBufferProgress(audioUrl, currentTime, duration) {
    try {
      ...
      const { totalChunks, fileSize } = audioInfo;
      let bufferedBytes = 0;
  ...
  getStats() {
    ...
    return {
      ...
      currentPlayingAudio: this.currentPlayingAudio
        ? this.currentPlayingAudio.title
        : null,
