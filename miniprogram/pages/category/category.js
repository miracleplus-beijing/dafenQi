...
Page({
  data: {
    ...
    featuredContent: [
      {
        ...
        podcast: {
          ...
          cover_url: 'https://gxvfcafgnhzjiauukssj.supabase.co/storage/v1/object/public/static-images/podcast_cover/defult_cover.png',
          duration: 45,
          channels: { name: 'AI前沿' }
        },
    ...
    rankings: {
      hot: [
        { id: 'h1', title: 'GPT-4技术解析：多模态大模型的突破', play_count: 15200 },
        { id: 'h2', title: '扩散模型原理与应用：从DALL-E到Stable Diffusion', play_count: 12800 },
  ...
  async initializePageData() {
    try {
      ...
      // 并行加载所有数据
      const [categoriesResult, featuredResult, rankingsResult] = await Promise.allSettled([
  ...
  handleDataLoadResults(results) {
    ...
    Object.entries(results).forEach(([key, result]) => {
      if (result.status === 'fulfilled' && result.value.success) {
        ...
        stats.failed++
        console.warn(`${key} 数据加载失败:`, result.reason || result.value?.error)
      }
    })
