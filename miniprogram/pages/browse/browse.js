...
Page({
  ...
  async loadWaterfallData(loadMore = false) {
    try {
      console.log('加载瀑布流数据', {
        loadMore,
        refreshing: this.data.waterfallRefreshing,
      });
      ...
      const result = await this.fetchPodcastsFromDatabase(1, {
        limit: batchSize,
      });
  ...
  // 瀑布流卡片交互由组件内部处理（Plan A），父页面不再接收事件
  ...
  handleWaterfallScrollToLower: function () {
    ...
    if (
      this.data.hasMoreWaterfallData &&
      !this.data.waterfallLoading &&
      !this.data.waterfallRefreshing
    ) {
